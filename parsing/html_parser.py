...
async def download_html_content(url, download_images=True,
    ...
    async with async_playwright() as p:
        ...
        try:
            ...
            def on_response(resp):
                if closing:
                    return
                tasks.append(asyncio.create_task(save_image_from_page_response(resp, img_dir=images_out_dir, saved_urls=img_urls)))
            if (download_images):
                create_and_clear_dir(images_out_dir)
            ...
            await auto_scroll(page, settings)
            try:
...
async def save_image_from_page_response(response, img_dir, saved_urls=None):
    ...
    try:
        ...
        img_ext = get_img_extension(content_type, url)
...
if __name__ == "__main__":
    ...
    r = asyncio.run(download_html_content(url=url, 
